...
function CountDownObject() {
    ...
    function TimerAlert() {
        ...
        // this.DIGITS_COLOR = "#ec1313"; 
        // CanvasContext.clearRect(0, 0, Canvas.width, Canvas.height);
        Canvas.setAttribute("style", "border-color: #e64545; box-shadow: 0 0 55px #e64545");
 
        // border: 10px solid hsla(0, 60%, 45%, 0.5); border-radius: 10px
        ...
        // CanvasContext.fillText("Alert", Canvas.width/2, 84.5*param.elementSize);
    }
    ...
    function DrawText() {
        ...
        if (num == 1) {
            CanvasContext.fillText("SECONDES", param.sec1Position + 80 * 0.5 * param.elementSize, 150 * param.elementSize);
        } else {
            if (removeNum == 1) offsetText = 1.1 * 80 * 0.5 * param.elementSize; // calculate offset inscriptions by using removeNum
            CanvasContext.fillText("SECONDES", param.sec1Position - 80 * 0.05 * param.elementSize - offsetText, 150 * param.elementSize);
        }
        if (num == 3) {
            ...
            CanvasContext.fillText("MINUTES", param.min1Position - 80 * 0.05 * param.elementSize - offsetText, 150 * param.elementSize);
        }
        if (num == 5) {
            CanvasContext.fillText("HEURES", param.hour1Position + 80 * 0.5 * param.elementSize, 150 * param.elementSize);
        } else if (num > 5) {
            ...
            CanvasContext.fillText("HEURES", param.hour1Position - 80 * 0.05 * param.elementSize - offsetText, 150 * param.elementSize);
        }
    ...
    function Main() {
        ...
        // if (timeAndDateNow.secs < 0) {
        //     timeAndDateNow.secs = 0;
        // }
        // if (timeAndDateNow.mins < 0) {
        //     timeAndDateNow.mins = 0;
        // }
        // if (timeAndDateNow.hours < 0) {
        //     timeAndDateNow.hours = timeAndDateNow.hours * -1;
        // }
        // if (timeAndDateNow.days < 0) {
        //     timeAndDateNow.days = timeAndDateNow.days * -1;
        // }
        // if (timeAndDateNow.months < 0) {
        //     timeAndDateNow.months = timeAndDateNow.months * -1;
        // }
        // if (timeAndDateNow.years < 0) {
        //     timeAndDateNow.years = timeAndDateNow.years * -1;
        // }
        ...
        //     (timeAndDateNow.mins == 5) &&
        //     (timeAndDateNow.secs == 0)) {
        //     // clearTimeout(timerId);
        //     // TimerEnd();

        //     //  timeAndDateLast.flagHours1 = DrawElement(param.hour1Position, 0, param.elementSize, param.elementSize, Math.floor(timeAndDateNow.hours%10), timeMillisecond, 9, true);

        //     TimerAlert();
        // }
        // if timer<0 start function
 
        if ($(CanvasContext.canvas).attr('id') == 'EndedCountDownCanvas') { 
         
            if ((timeAndDateNow.secs == 0) && $('#timer_end').val() == 'true'){

                var late_malus_value = $('#malus_late_value').val();
                var late_malus = (60 - timeAndDateNow.mins) * late_malus_value;
                $('#late_malus_timer span').html(late_malus);
            }
        }
        if ($(CanvasContext.canvas).attr('id') == 'CountDownCanvas') {
            if ($('#timer_end').val() == 'false') {
                if ((timeAndDateNow.days <= 0) &&
                    (timeAndDateNow.hours <= 0) &&
                    (timeAndDateNow.mins <= 0) &&
                    (timeAndDateNow.secs < 0)) {
    ...
    function resizeCanvas() {
        ...
        height = 117 * param.elementSize;
        // height = 150.1*param.elementSize;
